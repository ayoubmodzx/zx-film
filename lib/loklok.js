...
class LoklokClient {
  constructor(opts = {}) {
    ...
    // `keke` is the SEARCH CATALOG GATE (found 2026-08-22, decompiled interceptor
    // com.loklok.flash.android.net.o00oO0o line 291-295). keke='true' returns the
    // restricted/fuzzy catalog that HIDES Hollywood ("spider man" -> Panda Man,
    // The Other Man…); keke='false' returns the FULL catalog ("spiderman" ->
    // Spider-Man: Brand New Day, "batman" -> The Batman/Batman Begins, etc.).
    // It gates ONLY searchContent — detail/playInfo return identical results under
    // either value. NOT region-dependent (DZ egress returns Hollywood with keke=false).
    // The app flips keke via an internal pref, which is why a keke='true' capture
    // shows junk while the user's app (keke='false') shows Spider-Man.
